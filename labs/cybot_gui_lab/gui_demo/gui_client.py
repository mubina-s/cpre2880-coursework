...
DEFAULT_HOST = "192.168.1.1"   # CyBot IP
DEFAULT_PORT = 288             # CyBot port
...
class CyBotGui:
    ...
    def _build_ui(self):
        ...
        tk.Button(top, text="Connect", width=12, command=self.connect, bg="#f7c6d0", activebackground="#f9d6de").grid(row=0, column=4, padx=4)
        tk.Button(top, text="Disconnect", width=12, command=self.disconnect, bg="#f7c6d0", activebackground="#f9d6de").grid(row=0, column=5, padx=4)
        tk.Button(top, text="Request Scan", width=14, command=self.request_scan, bg="#f7c6d0", activebackground="#f9d6de").grid(row=0, column=6, padx=4)
        tk.Button(top, text="Hello", width=10, command=lambda: self.send_command("H"), bg="#f7c6d0", activebackground="#f9d6de").grid(row=0, column=7, padx=4)
        tk.Button(top, text="Status", width=10, command=lambda: self.send_command("S"), bg="#f7c6d0", activebackground="#f9d6de").grid(row=0, column=8, padx=4)
        ...
        tk.Button(cmd, text="Send", command=self.send_custom, bg="#f7c6d0", activebackground="#f9d6de").pack(side="left", padx=4)
        tk.Button(cmd, text="Save Scan As...", command=self.save_scan_as, bg="#f7c6d0", activebackground="#f9d6de").pack(side="left", padx=4)
        tk.Button(cmd, text="Reload From File", command=self.reload_from_file, bg="#f7c6d0", activebackground="#f9d6de").pack(side="left", padx=4)
        tk.Button(cmd, text="Clear Log", command=self.clear_log, bg="#f7c6d0", activebackground="#f9d6de").pack(side="left", padx=4)
        ...
        tips = (
            "1) Put your CyBot IP and port at the top.\n"
            "2) Click Connect one time.\n"
            "3) Click Hello or Status to test it.\n"
            "4) Click Request Scan to get the scan.\n"
            "5) The graph updates by itself."
        )
    ...
    def _draw_empty_plot(self):
        ...
        self.ax.grid(True, color="#f7c6d0")
    ...
    def _update_scan_data(self, angles: np.ndarray, distances: np.ndarray):
        ...
        self.ax.clear()
        self.ax.plot(np.deg2rad(angles), distances, linewidth=2.5, marker="o", markersize=3, color="#e75480")
        self.ax.set_title("CyBot Polar Scan")
    ...
    def _process_incoming(self):
        while True:
            ...
            if kind == "log":
                ...
                self.log(f"Scan complete. Points: {len(angles)}")
    ...
    def _network_worker(self):
        ...
        try:
            ...
            self.incoming.put(("status", f"Status: Connected to {host}:{port}"))
                ...
                if send_message.lower() == "m\n":
                    with open(SCAN_FILE, "w", encoding="utf-8") as scan_out:
                        while True:
                            ...
                            scan_out.write(line)
                            self.incoming.put(("log", f"Got: {line.strip()}"))
                    ...
                    self.incoming.put(("status", f"Status: Reply: {rx.decode(errors='replace').strip()}"))
